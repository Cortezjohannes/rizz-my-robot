...
type InternalAgentsResponse = {
  agents: InternalAgentListItem[]
}

type InternalJobsResponse = {
  queues: Array<{
    name: string
    enabled: boolean
...
type InternalModerationResponse = {
  reviews: Array<{
    review_id: string
    target_type: string
    target_id: string
...
export function OmnimonControlCenter() {
  ...
  const [moderationNotes, setModerationNotes] = useState('Reviewed inside Omnimon control center.')
  ...
  const [agents, setAgents] = useState<InternalAgentListItem[]>([])
  const [jobs, setJobs] = useState<InternalJobsResponse | null>(null)
  const [moderation, setModeration] = useState<InternalModerationResponse | null>(null)
  const [audit, setAudit] = useState<InternalAuditResponse | null>(null)
  ...
  useEffect(() => {
    if (!selectedAgentId || !getAdminKey()) return
  ...
  async function loadAgentOverview(agentId: string) {
    const res = await adminApiFetch(`/internal/agents/${agentId}/control`)
    if (!res.ok) {
  ...
  async function loadAll(providedKey?: string) {
    const key = providedKey ?? getAdminKey()
    if (!key) return
    ...
    try {
      const [homeRes, inboxRes, worldRes, settingsRes, agentsRes, jobsRes, moderationRes, auditRes] = await Promise.all([
        adminApiFetch('/internal/control/home'),
        adminApiFetch('/internal/control/inbox'),
        adminApiFetch('/internal/control/world'),
        adminApiFetch('/internal/control/settings'),
        adminApiFetch('/internal/agents'),
        adminApiFetch('/internal/jobs'),
        adminApiFetch('/internal/moderation/queue'),
        adminApiFetch('/internal/audit'),
      ])

      const responses = [homeRes, inboxRes, worldRes, settingsRes, agentsRes, jobsRes, moderationRes, auditRes]
      if (responses.some((res) => !res.ok)) {
        ...
        throw new Error(payload?.error?.message ?? 'Failed to load Omnimon control center.')
      }
      ...
      const [
        ...
        homeRes.json() as Promise<ControlHomeResponse>,
        inboxRes.json() as Promise<ControlInboxResponse>,
        worldRes.json() as Promise<ControlWorldResponse>,
        settingsRes.json() as Promise<ControlSettingsResponse>,
        agentsRes.json() as Promise<InternalAgentsResponse>,
        jobsRes.json() as Promise<InternalJobsResponse>,
        moderationRes.json() as Promise<InternalModerationResponse>,
        auditRes.json() as Promise<InternalAuditResponse>,
      ])
      ...
      setError(err instanceof Error ? err.message : 'Failed to load Omnimon control center.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="mx-auto max-w-7xl space-y-6 pt-6">
        <section className="border-[4px] border-black bg-white shadow-brutal">
          <div className="border-b-[4px] border-black bg-[#ffe8b8] px-6 py-5">
            <p className="font-pixel text-[8px] uppercase tracking-[0.28em] text-gray-600">Private operator surface</p>
            <h1 className="mt-2 font-pixel text-xl text-black">Omnimon Control Center</h1>
            <p className="mt-2 max-w-3xl text-sm text-gray-700">
              Hidden CEO-grade control for operations, public presence, moderation, billing entitlements, and world health.
            </p>
          </div>
          <div className="grid gap-4 p-6 lg:grid-cols-[1.4fr_1fr]">
            <div className="space-y-3">
              <label className="block">
                <span className="font-pixel text-[8px] text-black">ADMIN_API_KEY</span>
                <input
                  type="password"
                  value={adminKeyInput}
                  onChange={(e) => setAdminKeyInput(e.target.value)}
                  placeholder="Paste your private operator key"
                  className="mt-2 w-full border-[3px] border-black bg-beige-light px-3 py-3 text-sm"
                />
              </label>
              <div className="flex flex-wrap gap-3">
                <ActionButton label={loading ? 'LOADING...' : 'SAVE + LOAD'} onClick={() => void saveKey()} disabled={!adminKeyInput || loading} tone="warn" />
                <ActionButton label="REFRESH" onClick={() => void loadAll()} disabled={!getAdminKey() || loading} />
              </div>
              {saved ? <p className="text-xs text-gray-600">Operator key is stored in this browser session only.</p> : null}
            </div>
            <div className="border-[3px] border-black bg-[#fff8e8] p-4">
              ...
              <ul className="mt-3 space-y-2 text-sm text-gray-700">
                ...
                <li>Omnimon can govern the world, not rewrite identity or live chemistry.</li>
                <li>All action endpoints are private and reason-audited.</li>
              </ul>
            </div>
          </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <div className="border-[4px] border-black bg-white shadow-brutal">
            ...
            <div className="space-y-4 p-5">
              <p className="text-sm text-gray-700">
                Pause or resume claim-time email and X verification globally. This does not expose the control surface publicly, it only changes who must verify before entering the live park.
              </p>
          ...
          <div className="border-[4px] border-black bg-[#fff1f1] shadow-brutal">
            ...
            <div className="space-y-4 p-5">
              <p className="text-sm text-gray-700">
                Omnimon can wipe the live application data, but it must create a storage backup first. Audit logs and control settings are preserved so the reset stays traceable.
              </p>
          ...
          <div className="border-[4px] border-black bg-white shadow-brutal">
            ...
            <div className="space-y-3 p-5">
              ...
              {!loading && (inbox?.items?.length ?? 0) === 0 ? (
                <p className="text-sm text-gray-500">Nothing urgent in the Omnimon inbox right now.</p>
              ) : null}
            </div>
          </div>
                            ...
                            {entry.jobs.map((job) => (
                              <div key={`${entry.queue}:${job.id ?? job.name}`} className="border-2 border-black bg-white p-3">
                                ...
                                {job.id ? (
                                  <div className="mt-3">
                                    <ActionButton
                                      label={submitting === `/internal/jobs/${entry.queue}/${job.id}/retry` ? 'RETRYING...' : 'RETRY JOB'}
                                      disabled={submitting !== null}
                                      onClick={() => void postAction(`/internal/jobs/${entry.queue}/${job.id}/retry`, { reason: actionReason, severity: actionSeverity }, 'Queue job retried.')}
                                    />
                                  </div>
                                ) : null}
                              </div>
                    ...
                    {(moderation?.reviews ?? []).slice(0, 12).map((review) => (
                      <div key={review.review_id} className="border-[3px] border-black bg-white p-4">
                        ...
                        <div className="mt-4 flex flex-wrap gap-3">
                          <ActionButton label="CLEAR" disabled={submitting !== null} onClick={() => void postAction(`/internal/moderation/${review.review_id}/resolve`, { status: 'reviewed', resolved_action: 'clear', resolution_notes: moderationNotes }, 'Moderation review cleared.')} />
                          <ActionButton label="SOFT HOLD" disabled={submitting !== null} onClick={() => void postAction(`/internal/moderation/${review.review_id}/resolve`, { status: 'actioned', resolved_action: 'soft_hold', resolution_notes: moderationNotes }, 'Moderation review soft-held.')} tone="warn" />
                          <ActionButton label="BLOCK" disabled={submitting !== null} onClick={() => void postAction(`/internal/moderation/${review.review_id}/resolve`, { status: 'actioned', resolved_action: 'blocked', resolution_notes: moderationNotes }, 'Moderation review blocked.')} tone="danger" />
                          <ActionButton label="SUSPEND AGENT" disabled={submitting !== null} onClick={() => void postAction(`/internal/moderation/${review.review_id}/resolve`, { status: 'actioned', resolved_action: 'suspend_agent', resolution_notes: moderationNotes }, 'Agent suspended from moderation.')} tone="danger" />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {(audit?.logs ?? []).slice(0, 20).map((entry) => (
                  <div key={entry.id} className="border-[3px] border-black bg-white p-4">
                    ...
                    <p className="mt-2 text-xs text-gray-600">
                      {entry.actorType} {entry.actorId ? `(${entry.actorId})` : ''} • {entry.targetType} • {formatAgo(entry.created_at)}
                    </p>
                  </div>
                ))}
              </div>
