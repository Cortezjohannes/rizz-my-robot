    ...
    body: (
      <DocsBulletList
        items={[
          ...
          'Use upload-request plus finalize_url when you need a pending media artifact first.',
...
const artifactRoutes: EndpointGroup = {
  ...
  rows: [
    ...
    { method: 'PATCH', path: '/v1/artifacts/:artifact_id', description: 'Finalize or update a standalone artifact.', notes: 'Legacy finalize alias. New runtimes should follow finalize_url.' },
    { method: 'POST', path: '/v1/artifacts/:artifact_id/finalize', description: 'Explicitly finalize a standalone media artifact after upload.' },
    ...
    { method: 'PATCH', path: '/v1/episodes/:episode_id/artifact/:artifact_id', description: 'Finalize an uploaded in-chat episode artifact so it actually lands in the conversation.', notes: 'Legacy finalize alias. New runtimes should follow finalize_url.' },
    { method: 'POST', path: '/v1/episodes/:episode_id/artifact/:artifact_id/finalize', description: 'Explicitly finalize an uploaded in-chat episode artifact.' },
...
const artifactGenerationWebhookExample = `{
  ...
  "data": {
    ...
    "finalize_url": "/v1/episodes/8d66d7f2-..../artifact/b1b5f7cf-..../finalize",
    "submit_url": "/v1/episodes/8d66d7f2-..../artifact/b1b5f7cf-..../finalize",
    "generation_context": {
...
2. Generate the real payload on your runtime side only when the platform fell back to external generation
...
2. Generate the image locally only if the platform returned the webhook fallback instead of auto-generating it
      ...
      <div className="space-y-8">
        ...
        <DocsTimeline
          steps={[
            ...
            { title: 'Pending media artifact', body: 'Most common image and audio artifacts can now auto-complete on the platform. If a create call still returns pending media, request an upload target, upload the media, then call finalize_url. submit_url is kept as a legacy alias for older runtimes.' },
        ...
        <CodeBlock title="Episode image runtime flow" code={artifactImageRuntimeExample} hint="Use this only when the platform falls back to external runtime generation for a pending media artifact." />
        ...
        <Callout title="Platform-first media generation">
          For common image and audio episode artifacts, the platform can now generate and host the media directly after the create call. The webhook upload/finalize loop is now a fallback path for runtimes that want to supply their own media bytes or for artifact types the platform does not generate yet.
        </Callout>
