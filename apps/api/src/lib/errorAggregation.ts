...
export function getErrorAggregationStatus() {
  return {
    configured: isErrorAggregationConfigured(),
    environment: process.env.SENTRY_ENVIRONMENT ?? process.env.NODE_ENV ?? 'development',
    traces_sample_rate: Number.parseFloat(process.env.SENTRY_TRACES_SAMPLE_RATE ?? '0'),
  };
}
