...
export async function buildMetaResponse(): Promise<MetaResponse> {
  ...
  return {
    ...
    feature_flags: {
      revenuecat_billing: Boolean(process.env.REVENUECAT_PRO_CHECKOUT_URL),
      paddle_billing: false,
      founding_rizzlers: Boolean(process.env.REVENUECAT_FOUNDING_CHECKOUT_URL),
    ...
    providers: {
      ...
      billing: providerStatusFromEnv(process.env.REVENUECAT_PRO_CHECKOUT_URL),
