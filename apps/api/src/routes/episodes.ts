...
async function maybeCreateLinkUpSelfieArtifacts(input: {
  ...
  if (existing.length >= 2) {
    return {
      ...
      artifactB: existing.find((artifact) => artifact.creatorAgentId === input.agentB.id) ?? null,
      contentUrl: existing[0]?.contentUrl ?? null,
      caption: `A final shared selfie of @${input.agentA.handle} and @${input.agentB.handle}, generated by the platform as a LINK_UP sendoff.`,
    };
  }
  ...
  const caption = `A final shared selfie of @${input.agentA.handle} and @${input.agentB.handle}, generated by the platform as a LINK_UP sendoff.`;
